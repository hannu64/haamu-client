// PROTOCOL.md §7.4 — EFF short wordlist #1: 1296 words, every one 3-5 letters.
//
// ⚠️ §7.4 records that this is list #1 and NOT list #2, and the reason is not
// aesthetic. Short list #2 is chosen for its unique 4-character prefixes, which
// are worth having only in an autocompleting client — and D-021 rejects
// autocomplete, because it teaches every user that four letters identifies a
// word, and a four-letters-per-word note of an 8-word phrase retains 1.0 bit.
// List #2 also has the longest words of any EFF list (mean 7.3 letters), which
// D-019 measured as exactly the cost that matters.
//
// 1296 = 6^4, so a word is 10.34 bits: 8 words = 82.7 bits, 10 words = 103.4,
// before §7.4's cap of 60 shown candidates takes 5.91 bits off either figure.
//
// The list is data, not code, and it is FROZEN. Changing one word changes
// K_master for every phrase containing it, and there is no rotation (§7.2) —
// so this file may be read but never edited. Its checksum is asserted by the
// test suite:
//
//     SHA-256 of the words newline-joined, one trailing newline:
//     36ecca49e4fa20ca84b176c32f2e9c82f98f446585190e75f9879a95c08247bf
//
// Source: EFF's short wordlist #1, as used by the Phase 0.5 phrase test
// (haamu, `words/eff_short1.txt`), where D-019 measured 90% correct
// transcription at a median 24.4 s.
...
if (WORDS.length !== 1296) {
  throw new Error(`wordlist: expected 1296 words, got ${WORDS.length} — this file is frozen`);
}

/** The wordlist, frozen. Index i is the word for draw i. */
export const WORDLIST = Object.freeze(WORDS);

/** SHA-256 of the canonical newline-joined list, asserted by the tests. */
export const WORDLIST_SHA256 = "36ecca49e4fa20ca84b176c32f2e9c82f98f446585190e75f9879a95c08247bf";
